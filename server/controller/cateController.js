...
const cateController = {
  createCategory: async (req, res) => {
    try {
      ...
      return res
        .status(201)
        .json({ message: "Create category successfully", data: newCate });
    } catch (error) {
  ...
  updateCategory: async (req, res) => {
    try {
      ...
      const checkNameExist = await Category.findOne({
        name,
        _id: { $ne: cateId },
      });
      if (checkNameExist) {
      ...
      return res
        .status(200)
        .json({ message: "Update category successfully", data: updateCate });
    } catch (error) {
      return res.status(500).json(error.message);
    }
  },
  ...
  deleteCategory: async (req, res) => {
    try {
      ...
      if (isUsed) {
        return res
          .status(400)
          .json({ message: "Cannot delete category in use" });
      }
